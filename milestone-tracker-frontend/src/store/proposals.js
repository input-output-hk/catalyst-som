  ...
  actions: {
    ...
    async getProposal(id) {
      try {
        const { data, error } = await supabase
          .from('proposals')
          .select('*, challenges(*), allocations(*, users(id, user_id, email)), allocations_signoff(*, users(id, user_id, email)),  proposals_users(*, users(id, user_id, email)), change_request(id, created_at, url)')
          .eq('project_id', id)
        if (error) throw(error)
