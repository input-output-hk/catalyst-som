...
CREATE OR REPLACE FUNCTION public.getsubmittedsomreviews(_fund character varying, _from timestamp without time zone, _to timestamp without time zone)
 RETURNS TABLE(project_id bigint, title character varying, milestones_reviewed_qty bigint, latest_som_id bigint, latest_som_reviewed_at timestamp with time zone, email character varying, milestones_qty bigint)
 LANGUAGE plpgsql
AS $function$
  BEGIN
    RETURN QUERY

SELECT proposals.project_id, proposals.title, COUNT(proposals.project_id) as milestones_reviewed_qty, MAX(som_reviews.som_id) as latest_som_id, MAX(som_reviews.created_at) as latest_som_reviewed_at, users.email, proposals.milestones_qty
      from som_reviews
      INNER JOIN users
